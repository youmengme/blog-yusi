...
const markedRender = (
  ...
  const latexBlockParse = (latex: string) => {
    const html: string = katex.renderToString(latex, {
      displayMode: true,
      leqno: false,
      fleqn: false,
      throwOnError: false,
  ...
  const paragraphParse = (text: string) => {
    ...
    if (texBlock.test(text)) {
      ...
      while (texInline.test(text)) {
        const result: RegExpExecArray = texInline.exec(text)
        text = text.replace(result[0], latexInlineParse(result[2]))
      }
      return `<p>${text}</p>`
    } else if (emojiInline.test(text) && !excludeHttp.test(text)) {
      let back: string = ''
      const others: RegExpMatchArray = text.match(excludeColon)
      const textArray: Array<string> = text.replace('\n', ' ').split(' ')
      ...
      let textHandle: Array<string> = text
        ...
        .replace(markTag, '\n')
        .split('\n')
      const markTags: RegExpMatchArray = text.match(markTag)
  ...
  const codeParse = (code: string, language: string) => {
    if (language === 'diff') {
      const diffArray: Array<string> = code.split('\n')
      let backDiff: string = ''
